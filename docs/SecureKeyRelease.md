...
### Step 1 - Opening a Handle to the AziHSM Unwrapping Key
...
The first step of SKR is to open a handle to this unwrapping key.
...
Once a handle to the AziHSM's unwrapping key has been retrieved, the next step is to retrieve an **attestation report** for the key.
This data is used to attest and ensure that the AziHSM device the guest application is communicating with (and, by extension, the unwrapping key) can be trusted.
A guest application can do this via the AziHSM Guest Attestation Library.

![AziHSM SKR depiction](./.images/azihsm_skr_walkthrough_1_attestation_report.png)

### Step 3 - Attesting the AziHSM Unwrapping Key

To perform attestation of the AziHSM unwrapping key, the guest application next sends the attestation report to MAA (Microsoft Azure Attestation).
MAA verifies the report and issues a **MAA token** as proof of attestation.

![AziHSM SKR depiction](./.images/azihsm_skr_walkthrough_2_maa_token.png)
...
AKV/mHSM encrypts the contents of the requested key with the AziHSM unwrapping key (the public key).
...
![AziHSM SKR depiction](./.images/azihsm_skr_walkthrough_3_akv_release.png)
...
![AziHSM SKR depiction](./.images/azihsm_skr_walkthrough_4_key_unwrap.png)
